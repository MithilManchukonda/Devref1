...
export default function NoteModal({ open, onClose, onSave, user, subjects, editingSnippet }) {
  ...
  useEffect(() => {
    if (open && editingSnippet) {
      setSubject(editingSnippet.subject || "");
      setTitle(editingSnippet.title || "");
      setDesc(editingSnippet.desc || "");
      setCode(editingSnippet.code || "");
      setOutput(editingSnippet.output || "");
      setOutputImage(editingSnippet.outputImage || null);
    } else if (open) {
      resetForm();
    }
  }, [open, editingSnippet]);
  ...
  function resetForm() {
    setSubject(""); setTitle(""); setDesc(""); setCode(""); setOutput(""); setOutputImage(null);
  }
  ...
  async function handleSave() {
    ...
    if (!subject.trim() || !title.trim() || !code.trim()) { toast.error("Subject, title and code are required"); return; }
    setSaving(true);
    try {
      await onSave({
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold text-lg text-white">
                {isEditMode ? "Edit note" : "Add a cheat sheet note"}
              </h2>
            ...
            <div className="space-y-3.5">
              <div>
                <label className="text-[12px] text-[#a8b39e] block mb-1.5">Subject</label>
                <input
                  value={subject}
                  onChange={(e) => setSubject(e.target.value)}
                  placeholder="e.g. Python, DSA, SQL — your own category"
                ...
                <p className="text-[11px] text-[#5a6354] mt-1">Type any subject name — new ones show up as filter tabs automatically.</p>
              </div>
              <div>
